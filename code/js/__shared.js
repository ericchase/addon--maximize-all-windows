...
function maximize_all_windows() {
  ...
  browser.windows.getAll({windowTypes: ['normal', 'popup', 'panel', 'devtools']})
    .then(maximize_all)
    .then(minimize_necessary)
    .then(bring_back_focus)
    .catch(error => console.log('[__shared.js] maximize_all_windows: error:', error))
}

//  Each of these functions acts like part of a stream. No matter what happens, they return the arguments they receive
//  without changing them. By doing this, it becomes very easy to chain multiple functions together that alter the
//  environment, but not the arguments.
...
function maximize_all( windows ) {
  ...
  return new Promise(function ( resolve ) {
    return Promise.all(windows
      ...
      .map(maximize))
      .then(() => resolve(windows)).catch(() => resolve(windows))
  })
}
...
function minimize_necessary( windows ) {
  ...
  return new Promise(function ( resolve ) {
    return browser.storage.local.get('minimize-after-action')
      .then(result => {
    ...
    return Promise.all(windows
      ...
      .map(window =>
        browser.windows
          ...
          .then(window => console.log('[__shared.js] info: focused:', window.id))
          .catch(error => console.log('[__shared.js] bring_back_focus: error:', error)),
      )).then(() => resolve(windows)).catch(() => resolve(windows))
  })
}
...
//
//  return value -> A Promise that will be fulfilled with a windows.Window object containing the details of the
//                  updated window. If any error occurs, the promise will be rejected with an error message.
//
//  updateInfo: object
//  {
